...
def create_pedido(datos: dict):
    try:
        if not datos:
            raise HTTPException(status_code=500, detail=f"Error al crear el pedido")
...
def update_pedido(pedido_id: int, datos: dict):
    try:
        if not datos:
           raise HTTPException(status_code=500, detail=f"Error al actualizar el pedido")
        datos=jsonable_encoder(datos)
